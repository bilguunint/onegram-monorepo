...
export class LayoutComponent implements OnInit, AfterViewInit {
  ...
  /**
   * Check user role and redirect manager to users page
   */
      ...
      firebase.auth().onAuthStateChanged(async (user) => {
        if (user) {
          ...
          if (adminDoc.exists) {
            ...
            console.log('Layout - Admin role:', adminRole);
            console.log('Layout - Current URL:', currentUrl);

            // Manager эрхтэй бол users component-руу чиглүүлэх
            if (adminRole === 'manager') {
              if (currentUrl === '/' || currentUrl === '/dashboard' || currentUrl === '') {
                console.log('Layout - Redirecting manager to users');
