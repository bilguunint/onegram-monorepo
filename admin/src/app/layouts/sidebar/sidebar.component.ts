...
export class SidebarComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit() {
    // Firebase auth state шалгаж админы role авах
  ...
  filterMenuByRole(menuItems: MenuItem[]): MenuItem[] {
    if (this.adminRole === 'manager') {
      // Manager эрхтэй админд зориулсан menu filter
      const filteredItems = menuItems.filter(item => {
        // "Эхлэл" menu нуух (id: 10)
        if (item.id === 10) {
          return false;
        }
        return true;
      ...
      // "Хэрэглэгчид" menu-г эхэнд тавих
      return filteredItems.sort((a, b) => {
        if (a.id === 11) return -1; // Хэрэглэгчид menu эхэнд
        if (b.id === 11) return 1;
    ...
    // Admin болон бусад эрхийн хувьд бүх menu харуулах
    return menuItems;
  }
  ...
  initialize(): void {
    ...
    this.menuItems = this.filterMenuByRole(originalMenu);
